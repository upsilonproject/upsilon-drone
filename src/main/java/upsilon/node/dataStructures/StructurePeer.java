...
import upsilon.node.management.rest.client.RestClient;
...
public class StructurePeer extends ConfigStructure {
	...
	public final static void updateAll() {
		...
		for (final StructurePeer p : Configuration.instance.peers) {
			...
			if (clientService == null) {
				...
				for (final StructureService s : upsilon.node.Configuration.instance.services) {
	...
	private void newClient() throws MalformedURLException, IllegalArgumentException, GeneralSecurityException {
		if (this.restClient == null) {
			...
			StructurePeer.LOG.info("Crypo status on this node is: " + Configuration.instance.isCryptoEnabled + " . All communication with peers will be via: " + proto);
	...
	@Override
	public void update(final ConfigurationNode<?> xmlNode) {
